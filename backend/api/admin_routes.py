...
@router.get(
    ...
    summary="Список вентиляторов (админ)",
)
def admin_products(
...
@router.get(
    ...
    summary="Карточка вентилятора (админ)",
)
def admin_product_get(
    product_id: str,
    _: Annotated[dict, Depends(require_admin)],
):
    from db.repository import get_by_id
...
@router.post(
    ...
    summary="Удалить несколько вентиляторов",
)
def admin_products_bulk_delete(
    payload: BulkDeleteProductsIn,
    _: Annotated[dict, Depends(require_admin)],
):
    with db_session() as conn:
...
@router.get(
    "/users",
    response_model=UserListPageOut,
...
def admin_users(
    ...
    return UserListPageOut(items=[UserOut.model_validate(u) for u in items], total=total, limit=limit, offset=offset)
