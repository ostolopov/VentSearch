...
from infrastructure.db.product_repository import fetch_all_products_dicts
from infrastructure.search.bloom_filter import BloomFilter
from infrastructure.search.numeric_index import SortedRangeIndex
...
def _airflow_ok(r: dict, min_airflow: Optional[float], max_airflow: Optional[float]) -> bool:
    af = r["airflow"]
...
def _pressure_ok(r: dict, min_p: Optional[float], max_p: Optional[float]) -> bool:
    pr = r["pressure"]
...
class CatalogIndex:
    __slots__ = (
        "_rows", "_by_id",
        "_type_bloom", "_size_bloom",
        "_type_to_ids", "_size_to_ids",
        "_idx_price", "_idx_power", "_idx_noise", "_idx_diameter",
    )
    ...
    def build(cls, conn) -> "CatalogIndex":
        ...
        def pairs(getter) -> List[tuple]:
            return [(float(v), r["id"]) for r in rows if (v := getter(r)) is not None]
        ...
        logger.info(
            "Поисковый индекс: %s записей, Bloom(type) ~%s, Bloom(size) ~%s",
            len(rows), len(distinct_types), len(distinct_sizes),
        )
        return cls(rows, type_bloom, size_bloom, type_to_ids, size_to_ids,
                   idx_price, idx_power, idx_noise, idx_diameter)

    @staticmethod
    def _intersect(cur: Optional[Set[str]], nxt: Set[str]) -> Set[str]:
        return set(nxt) if cur is None else cur & nxt

    def search(
        self,
        *,
        q: Optional[str], type_: Optional[str], series: Optional[str],
        diameter: Optional[float],
        min_price: Optional[float], max_price: Optional[float],
        min_power: Optional[float], max_power: Optional[float],
        min_noise: Optional[float], max_noise: Optional[float],
        min_diameter: Optional[float], max_diameter: Optional[float],
        min_airflow: Optional[float], max_airflow: Optional[float],
        min_pressure: Optional[float], max_pressure: Optional[float],
        sort: str, limit: int, offset: int,
    ) -> Tuple[List[dict], int]:
        if not self._rows:
            return [], 0

        cur: Optional[Set[str]] = None

        if type_:
            if not self._type_bloom.might_contain(type_):
                return [], 0
            cur = set(self._type_to_ids.get(type_, set()))
            if not cur:
                return [], 0
        ...
        candidates = list(self._rows) if cur is None else [self._by_id[i] for i in cur if i in self._by_id]
        ...
        if sort == "price_desc":
            out.sort(key=lambda r: (r["price"] is None, -(r["price"] or 0) if r["price"] is not None else 0, r.get("model") or ""))
        else:
            out.sort(key=lambda r: (r["price"] is None, r["price"] if r["price"] is not None else 0.0, r.get("model") or ""))
