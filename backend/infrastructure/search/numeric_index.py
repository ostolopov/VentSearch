"""
Отсортированные индексы для числовых полей: поиск диапазона через bisect
(логика B-tree / упорядоченный индекс).
"""
import bisect
...
class SortedRangeIndex:
    """Пары (значение, id), отсортированные по значению."""
    ...
    def ids_in_range(
        ...
        """Идентификаторы записей, у которых значение попадает в [min_v, max_v] включительно."""
        if not self._pairs:
            return set()
