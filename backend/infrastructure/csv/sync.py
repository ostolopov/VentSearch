"""
Синхронизация каталога с CSV: дешёвая проверка по mtime + размеру,
SHA-256 только при расхождении метаданных.
"""
import hashlib
import logging
...
def sync_catalog_from_csv(conn, csv_path: Path) -> bool:
    ...
    Быстрый путь: совпадение сохранённых mtime_ns и size_bytes — без чтения файла.
    При расхождении — SHA-256; совпадает — обновить только mtime/size.
    Возвращает True, если выполнялась перезагрузка данных.
    """
    ...
    if (
        state
        and state["csv_path"] == path_str
        and state["mtime_ns"] == mtime_ns
        and state["size_bytes"] == size_bytes
    ):
        return False
    ...
    if state and state["csv_path"] != path_str:
        logger.info("CSV_PATH сменился (%s -> %s), полная перезагрузка", state["csv_path"], path_str)
    ...
    logger.info("Содержимое CSV изменилось (хеш не совпадает), перезагрузка каталога")
    _reload_from_csv(conn, resolved, path_str, mtime_ns, size_bytes, sha256_hex=current_hash)
    return True
